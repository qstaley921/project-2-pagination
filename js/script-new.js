...
function popPage() {
  ...
  for (let i = 0; i < students.length; i++) {
    ...
    if (studentName.includes(searchValue) === false) { // if name != search bar value
      filtered += 1; // item is considered filtered
      console.log(filtered);
    }
    if (studentName.includes(searchValue)) { // if name === search bar value
      if ( (i - filtered) <= (aBtn * 9) && (i - filtered) > ((aBtn -1) * 9) ) { // guarantees only 9 items are ever printed
        genItem(i, studentName);
      }
    }
  }
